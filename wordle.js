...
function enterWord(word) {
  console.log("in enterWord. word =", word);
  for (let letter of word) {
    // console.log("keydown for letter ", letter);
    window.dispatchEvent(new KeyboardEvent("keydown", { key: letter }));
  }
  //   console.log("keydown for Enter");
  window.dispatchEvent(new KeyboardEvent("keydown", { key: "Enter" }));

  // wait for some seconds and check what all are correct. Filter and enterword again.
  ...
  if (trialNumber < 6) {
    ...
    setTimeout(() => {
      ...
      filteredWords = filteredWords.filter((filteredWord) => {
        ...
        // filtered word shouldn't contain any blacklisted characters
        for (letterInFilteredWord of filteredWord) {
          if (blackListedLetters[letterInFilteredWord]) {
            isWordValid = false;
          }
        }

        // filtered word should definitely contain whitelisted characters
        if (isWordValid) {
          Object.keys(whiteListedLetters).forEach((whiteListedLetter) => {
            if (!filteredWord.includes(whiteListedLetter)) {
              isWordValid = false;
            }
          });
        }
        ...
        if (isWordValid) {
          ...
          for (
            ...
            if (
              targetWord[letterIndex] !== null &&
              targetWord[letterIndex] !== filteredWord[letterIndex]
            ) {
              isWordValid = false;
            }
          }
      ...
      console.log("filtered words. Filtered length:", filteredWords.length);
      guessWord =
        filteredWords[Math.floor(Math.random() * filteredWords.length)];
      trialNumber += 1;
      console.log(
        "calling enterWord again with the following word:",
        guessWord
      );
      enterWord(guessWord);
    }, 2000);
